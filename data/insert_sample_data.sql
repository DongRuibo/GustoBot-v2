...
INSERT INTO ingredients (
    ...
    '五花肉', '肉类-原料', 395.00, 13.20, 0.00, 37.00, '冷藏', 2
);
SELECT id INTO @ing_pork_belly FROM ingredients WHERE name = '五花肉';
...
INSERT INTO recipes (
    ...
    '红烧肉',
    '经典家常红烧肉，五花肉先焯水再煸出油脂，小火焖至软糯入味。',
    'https://images.example.com/hongshao_rou.jpg',
    'https://videos.example.com/hongshao_rou.mp4',
    75, 3, 'medium', @cuisine_home,
    1160.00, 42.00, 28.00, 88.00
);
SET @recipe_braised_pork := LAST_INSERT_ID();

INSERT INTO recipe_steps (
    recipe_id, step_number, action, instruction, duration, temperature, tools_used, tips
) VALUES (
    @recipe_braised_pork, 1, '焯水',
    '五花肉切成约3厘米方块，冷水下锅，加入姜片和料酒，大火煮沸后撇去浮沫并捞出沥干。',
    10, '大火', JSON_ARRAY('汤锅', '砧板', '菜刀'),
    '冷水下锅更容易去除血沫，焯水后沥干可以减少煸炒时溅油。'
);
SET @step_braised_pork_1 := LAST_INSERT_ID();

INSERT INTO step_tools (step_id, tool_id, `usage`) VALUES
    (@step_braised_pork_1, @tool_soup_pot, '焯水'),
    (@step_braised_pork_1, @tool_board, '切块'),
    (@step_braised_pork_1, @tool_knife, '切块');

INSERT INTO recipe_steps (
    recipe_id, step_number, action, instruction, duration, temperature, tools_used, tips
) VALUES (
    @recipe_braised_pork, 2, '煸炒上色',
    '炒锅中放少量油，下五花肉块煸至表面微黄，再加入白砂糖小火炒化，倒入酱油翻匀上色。',
    8, '中小火', JSON_ARRAY('炒锅', '锅铲'),
    '糖色不宜炒过头，颜色变琥珀色即可加入肉块翻匀。'
);
SET @step_braised_pork_2 := LAST_INSERT_ID();

INSERT INTO step_tools (step_id, tool_id, `usage`) VALUES
    (@step_braised_pork_2, @tool_wok, '煸炒'),
    (@step_braised_pork_2, @tool_spatula, '翻炒');

INSERT INTO recipe_steps (
    recipe_id, step_number, action, instruction, duration, temperature, tools_used, tips
) VALUES (
    @recipe_braised_pork, 3, '焖煮收汁',
    '加入热水没过肉块，放入葱段、姜片、料酒和少许盐，小火焖煮50分钟，最后大火收至汤汁浓稠。',
    50, '小火', JSON_ARRAY('汤锅', '锅铲'),
    '焖煮阶段保持小火，收汁时轻轻翻动，避免肉块散碎。'
);
SET @step_braised_pork_3 := LAST_INSERT_ID();

INSERT INTO step_tools (step_id, tool_id, `usage`) VALUES
    (@step_braised_pork_3, @tool_soup_pot, '焖煮'),
    (@step_braised_pork_3, @tool_spatula, '收汁');

INSERT INTO recipe_ingredients (
    recipe_id, ingredient_id, quantity, unit, prep_method, prep_time,
    is_main, substitute, adjusted_calories, ingredient_type
) VALUES
    (@recipe_braised_pork, @ing_pork_belly, '500', 'g', '切块焯水', 12, 1, '猪肉里脊', 1975.00, 'main'),
    (@recipe_braised_pork, @ing_ginger, '15', 'g', '切片', 2, 0, NULL, 12.00, 'seasoning'),
    (@recipe_braised_pork, @ing_scallion, '20', 'g', '切段', 2, 0, NULL, 6.40, 'seasoning'),
    (@recipe_braised_pork, @ing_soy_sauce, '25', 'ml', '调味上色', 1, 0, NULL, 18.00, 'seasoning'),
    (@recipe_braised_pork, @ing_sugar, '20', 'g', '炒糖色', 0, 0, NULL, 77.40, 'seasoning'),
    (@recipe_braised_pork, @ing_shaoxing_wine, '25', 'ml', '去腥', 1, 0, NULL, 13.00, 'seasoning'),
    (@recipe_braised_pork, @ing_salt, '3', 'g', '调味', 0, 0, NULL, 0.00, 'seasoning'),
    (@recipe_braised_pork, @ing_cooking_oil, '10', 'g', '煸炒', 0, 0, NULL, 88.40, 'seasoning');
